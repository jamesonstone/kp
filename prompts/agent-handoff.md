...
---
Create a lossless, zero-context handoff from the current coding-agent session to a different coding-agent system. The destination may use different models, tools, shells, repository integrations, and instruction formats. Preserve facts and authority, not provider-specific assumptions. Completeness means recording known state and native evidence states; do not delay emission to obtain unreachable artifacts. Remove redundancy before removing detail.
...
- Unreachable attachments, Drive links, recordings, screenshots, and other third-party files are not blocking questions. Record the original URL or identifier, write `UNKNOWN` or `BLOCKED`, and continue. Do not ask the user to attach, re-attach, download, or grant access.
- Capture time versus a commit, deploy, or pull request is not a blocking origin question. Record provenance as `UNKNOWN` and let the destination inspect live product and repository state.
- Ask the user only when the unanswered fact would change destination authority, the landing lane, or the remaining-work contract, and cannot be recorded as `UNKNOWN`, `CONFLICT`, or `BLOCKED` with one bounded destination inspection. Then output only the smallest numbered set of questions and stop. Do not produce the handoff in the same response.
- If the user instructs to skip questions, emit, or output the handoff, emit immediately with remaining items in native evidence states.
- Do not mutate files, Git state, issues, pull requests, deployments, or external systems solely to prepare the handoff, including helper scripts used to fetch evidence.

## Origin phase 2: emit

Begin only after phase 1 has zero unresolved blocking questions, or after the user has instructed to emit. Refresh volatile Git, review, check, deployment, and runtime evidence once, then return only one fenced code block with language tag `markdown`; place no text outside it. The document must be titled `# Coding Agent Handoff` and use these H2 sections in order:
...
- Preserve native evidence states such as `PENDING`, `UNKNOWN`, `SKIPPED`, `BLOCKED`, `NOT_RUN`, and `NOT_APPLICABLE`. Never convert them into success. Never convert missing or unreachable evidence into a user retrieval request.
...
- Acceptance and Validation: use binary-verifiable criteria. Map each criterion to exact commands, suites, hosted checks, environment, expected literal result, and current evidence state tied to a commit or artifact when that identity is already known. Do not require third-party capture files to satisfy this mapping.
...
1. Clarify: treat the handoff as an attributable snapshot, load destination and repository-local instructions, inspect live state read-only, verify paths/symbols/heads/checks/artifacts that exist in the live workspace, detect drift, and ask only questions that would change destination authority, the landing lane, or the remaining-work contract and cannot be answered from the handoff or inspection. Do not ask the user to supply unreachable third-party files or confirm capture time versus a commit. Output only those questions and stop. Do not repeat completed work.
