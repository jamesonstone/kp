...
# Definition of done
A work unit is not complete until the latest punch list has been re-scanned, affected items are understood, root cause is established, implementation and relevant tests are complete, review feedback is resolved, the change is merged and deployed to the validation environment, affected items contain accurate engineering notes, validators have clear re-test instructions when needed, and subsequent validation feedback is incorporated. Code completion alone is not completion.
